...
public class SplashActivity extends AppCompatActivity {
    ...
    public void checkReceiverphonePermission(){
        int permissionCheck =ContextCompat.checkSelfPermission(SplashActivity.this,Manifest.permission.READ_CALL_LOG);
        if(permissionCheck==PackageManager.PERMISSION_GRANTED) {
            checkStoragePermission();
        }else{
            ActivityCompat.requestPermissions(SplashActivity.this,new String[]{Manifest.permission.READ_CALL_LOG},Global.REQ_CODE_PERMISSION_PHONENUMBER);
        }
    ...
    public void goToContentsActivity() {
        ...
        sharedPreferences.edit().putInt(Global.SHARED_PREFERENCES_WIDTH, full_width).apply(); // 가로 해상도의 7/10
        sharedPreferences.edit().putInt(Global.SHARED_PREFERENCES_HEIGHT, full_height).apply(); // 세로 해상도의 4/10
        ...
        if (!isServiceRunning()) {
            ...
            // 안드로이드 8.0 이상에서는 ForegroundService 실행
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        } else if(requestCode==Global.REQ_CODE_PERMISSION_PHONENUMBER){
